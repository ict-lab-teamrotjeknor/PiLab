...
/*
 * Create a new api client with a specified size response buffer.
 *
	...
	api_client_create_with_max_response_size(size_t max_size)
{
	...
	new_client->max_size_reponse_buffer = max_size;
...
{
	return api_client_create_with_max_response_size(MAX_RESPONSE_BUFFER);
}
