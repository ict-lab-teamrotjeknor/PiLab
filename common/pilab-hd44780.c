 ...
 *
 * Currently this is the only way of interfacing with the device.
 */
...
void hd44780_backlight(struct hd44780 *lcd, int pin, int ll)
{
	...
	if (lcd->i2c_device) {
		ext_pin = i2c_device_ext_pin(lcd->i2c_device, pin);
...
void hd44780_relative_write(struct hd44780 *lcd, const char *msg)
{
	...
	rw = i2c_device_ext_pin(lcd->i2c_device, 0x01);
	ca = i2c_device_ext_pin(lcd->i2c_device, 0x03);
	pinMode(rw, OUTPUT);
	pinMode(ca, OUTPUT);
	digitalWrite(rw, LOW);
